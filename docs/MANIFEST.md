...
build prerequisite. The listed downloads stay outside this repository entirely;
the only local input with a repository path is the ignored
`docs/instruction-site/` guide snapshot. Their absence does not block normal
`just` verification; only an action that explicitly needs a listed original
input may require the user to provide and check it. Environment ownership and
the capability boundary are defined in [`docs/environment.md`](environment.md).
...
The course download center published these slides on 2026-07-23. They are kept
in the user's local material set outside this repository:
...
The Lab 2 material set is kept outside this repository. For the selected
miniRV + EGO1 path it contains local snapshots of the guide assets that are used
outside the guide itself:

| File | SHA-256 | Purpose | Source in guide snapshot |
| --- | --- | --- | --- |
| `bin2coe-guide.py` | `faafd96a730cb28b170c9ad9f96f7c9d3d67c614311060007b89864f28bf9cc4` | Unmodified guide snapshot of the BIN-to-COE converter | `lab2-A/assets/bin2coe.py` |
...
| `c_test_rv_stu.tar.gz` | `1b685c2334d2fac3d1f188ab1c420b670af3b37a3e3bb7748efe7ebc51612686` | miniRV I/O, DDR, CoreMark, and LLAMA2 test programs | `lab2-B/assets/c_test_rv_stu.tar.gz` |
| `lab2_IOtest_miniRV_ego1.bit` | `fcbbfe3815c278050f4fdf51585da8863c90a53bf55818aa3879b3c2b26130ac` | User-owned EGO1 validation of completed C_TEST programs | `lab2-B/assets/lab2_IOtest_miniRV_ego1.bit` |
...
`lab2-B/assets/bin2coe.py`. Preserve `bin2coe-guide.py` byte-for-byte for
...
| Path | Upstream | Branch | Commit | Purpose |
| --- | --- | --- | --- | --- |
| `tests/cdp/` | `https://gitee.com/hitsz-cslab/cdp-tests.git` | `miniRV` | `50a818278e9a60d304521c4b16980211b0162014` | Trace framework with repaired `start` test |
| `docs/instruction-site/` | `https://git.cs-lab.top/cslab/cpu-design.git` | `main` | `e2748d2b7cd765a19146dff1355cc842ac68fe64` | Official course guide with Lab 2-A and Lab 2-B |

Neither path is a Git submodule any more. `tests/cdp/` is vendored into this
repository at the commit above; `docs/instruction-site/` is an ignored local
snapshot of the guide's `docs/` tree, flattened so that guide section paths such
as `lab2-B/0-overview.md` are relative to it. Refresh either one explicitly
after reviewing the upstream diff and record the new commit here; do not
automatically track the latest upstream commit.
