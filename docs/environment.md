...
本页是本仓库环境能力的唯一合同入口。它说明每项能力由谁提供、在哪里配置、用哪个公开命令验证，以及缺失时影响什么；课程行为的具体合同以 `docs/instruction-site/` 本地快照的 live source 为准，Trace 合同以 vendored 的 `tests/cdp/` 为准。

## 能力边界

| 能力 | Owner 与版本控制 | 最小前提和配置位置 | 验证入口 | 缺失时的影响 |
| --- | --- | --- | --- | --- |
| Linux 日常开发与自动验证 | `.devcontainer/`，以及 vendored 的 `tests/cdp/`；均进入 Git | 在 devcontainer 中打开仓库 | `just doctor`，随后按需执行 `just lint`、`just trace`、`just unit`、`just integration`、`just system` 或 `just gate` | 缺少必需工具或仓库输入时，`just doctor` 失败；这不是 RTL 或产品行为失败 |
| 官方指导书快照 | 用户本地的 ignored `docs/instruction-site/`；provenance 和 commit 记录在 `docs/MANIFEST.md` | 由用户放置到 `docs/instruction-site/`，不随 clone 获得 | `just doctor` 的 Repository inputs 检查 `docs/instruction-site/index.md`；按 `docs/MANIFEST.md` 人工核对来源 | 缺失时 `just doctor` 报告 `[missing]` 并失败；lint、Trace、unit、integration、system 本身不读取它 |
...
`just doctor` 的 Required tools、RV32IM/ILP32 probe 与 Repository inputs 是这条路径的 fail-closed 检查。`tests/cdp/` 是 Trace 的序列化后端，不是第二个公开 CLI；所有日常命令仍从根 `Justfile` 进入。

## 受限材料

`docs/MANIFEST.md` 记录可取得的原始课程下载物和用户快照的来源、用途与 SHA-256。它不是下载器、镜像构建输入或“每次构建前必须补齐”的清单。不要自动下载、复制、提交或重分发 ignored 的 `docs/instruction-site/` 快照，也不要将它的路径写入 `local.env`。

日常 `just lint`、Trace、unit、integration、system 和 closure 只使用 tracked 产品源、测试和 vendored 的 `tests/cdp/`；已导入的程序和维护中的 RTL 也不在运行时读取该 ignored 目录。某个历史材料确实被要求时，先按 manifest 核对其 provenance/hash，再在该动作的记录中说明其使用；材料不存在本身不是 RTL、Trace 或产品失败。
...
`just doctor` 的 Vivado 段是 capability 报告：`[not configured]` 或 `[not available]` 不改变核心 Linux 开发路径的成功状态。相应的 Vivado 消费者才会在缺少前提时退出；不要用临时脚本绕过根 `Justfile`。
