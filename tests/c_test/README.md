...
runs it from reset PC `0x00000000` through the canonical single-cycle CPU,
enabled I/D caches, AXI master, product fabric, behavioral memory, and MMIO.
The testbench drives and decodes only the top-level UART `rx`/`tx` pins for
program interaction. Internal buses are observed for protocol assertions but
are not used to inject program state.
